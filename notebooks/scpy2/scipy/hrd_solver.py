...
class HrdSolver(object):
    ...
    def __init__(self, status):
        ...
        self.fig.canvas.set_window_title('华容道求解器')
    ...
    def on_key(self, event):
        if event.key == " ":
            ...
            for rect in list(self.rectangles.values()):
                rect.remove()
            self.empty_pos = set(product(list(range(H)), list(range(W))))
    ...
    def get_blocks(self, x, y):
        h, w = SIZES[self.current_block_type]
        return {(y + dy, x + dx) for dy, dx in product(list(range(h)), list(range(w)))}

    def can_place_block(self, x, y):
        return self.get_blocks(x, y).issubset(self.empty_pos)

    def get_status(self):
        name_map = {v: k for k, v in list(SIZES.items())}
        board = [" "] * (W * H)
        for pos, rect in list(self.rectangles.items()):
