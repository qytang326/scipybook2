...
Options = {
    "以种子为标准-4联通": cv2.FLOODFILL_FIXED_RANGE | 4,
    "以种子为标准-8联通": cv2.FLOODFILL_FIXED_RANGE | 8,
    "以邻点为标准-4联通": 4,
    "以邻点为标准-8联通": 8
}
...
class FloodFillDemo(ImageProcessDemo):
    ...
    point = Tuple((0, 0))
    option = Trait("以邻点为标准-4联通", Options)
    ...
    def control_panel(self):
        return VGroup(
            Item("lo_diff", label="负方向范围",  width=-20, editor=TupleEditor(cols=2)),
            Item("hi_diff", label="正方向范围", editor=TupleEditor(cols=2)),
            Item("fill_color", label="填充颜色", editor=TupleEditor(cols=2)),
            Item("point", label="种子坐标", editor=TupleEditor(cols=2, labels=["x", "y"])),
            Item("option", label="算法标志")
        )
