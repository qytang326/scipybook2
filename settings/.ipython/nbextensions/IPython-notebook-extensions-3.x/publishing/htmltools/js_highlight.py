...
try:
    ...
    from html.parser import HTMLParser  # py2
...
class HtmlHighlightStripper(HTMLParser):
    ...
    def handle_starttag(self, tag, attrs):
        ...
        if tag == "pre":
            ...
            cssclass = attrs_dict.pop("class", None)
            if cssclass and cssclass.startswith("hl-"):
                self.pygments_fix = True
                tag = "div"+self.stringify_attrs(attrs_dict)+">"
                tag += "<pre><code"
        ...
        if _print:
            self.out("<%s%s>" % (tag, self.stringify_attrs(attrs)))
    ...
    def handle_endtag(self, tag):
        ...
        if tag == "pre":
            ...
            _print = False
        elif tag == "div" and self.pygments_fix:
            tag = "code></pre></div"
            self.pygments_fix = False
