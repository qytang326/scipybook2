...
class DataSource(HasTraits):
    ...
    def load_csv(self, filename):
        ...
        f.close()
        for name, coldata in zip(self.names, array.T):
...
class Graph(HasTraits):
    ...
    clear_button = Button("清除") # 快速清除Y轴的所有选择的数据
        ...
        HSplit( # HSplit分为左右两个区域，中间有可调节宽度比例的调节手柄
            ...
            VGroup(
                ...
                Item("selected_xaxis", editor=
                    EnumEditor(name="object.data_source.names", format_str="%s")),
                Heading("Y轴数据"), # 静态文本
...
class CSVGrapher(HasTraits):
    ...
    csv_file_name = File(filter=["*.csv"]) # 文件选择
    add_graph_button = Button("添加绘图") # 添加绘图按钮
    ...
    view = View(
        ...
        VGroup(
            ...
            HGroup(
                # 文件选择控件
                Item("csv_file_name", label="选择CSV文件", width=400),
                # 添加绘图按钮
