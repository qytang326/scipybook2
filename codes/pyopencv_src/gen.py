...
def gen(name, args, ty, flags):
  ...
  if 'doconly' in flags:
    ...
    for a in args:
      ...
      if 'O' in a.flags:
        continue
      if a.ty in (conversion_types + list(aggregate.keys())):
    ...
    for k in (conversion_types + list(aggregate.keys())):
      fmap[k] = 'O'
...
for nm,args,ty,flags in sorted(api):
  ...
  print(entry % (nm), file=gen_c[1])
  if nm.startswith('CV_'):
    print(entry % (nm[3:]), file=gen_c[1])
  for l in gen(nm,args,ty,flags):
    print(l, file=gen_c[0])

for l in open("%s/defs" % sys.argv[1]):
  print("PUBLISH(%s);" % l.split()[1], file=gen_c[2])
...
for (t, flags, members) in objects:
    ...
    if 'copy' in flags:
        ...
        a = '->'
    gsf = "".join([getset_func_template.substitute(map, accessor = a, member = m, checker = checkers[t], converter = converters[t], rconverter = rconverters[t], typename = typenames[t]) for (m, t) in list(members.items())])
    # gsi is the generated code for the initializer for each accessor
    gsi = "".join([getset_init_template.substitute(map, member = m) for (m, t) in list(members.items())])
    ...
    if 'allownull' in flags:
        ...
        nullcode = ""
    if 'copy' in flags:
        print(gensimple.substitute(map, getset_funcs = gsf, getset_inits = gsi, allownull = nullcode), file=gen_c[3])
    else:
        print(genptr.substitute(map, getset_funcs = gsf, getset_inits = gsi, allownull = nullcode), file=gen_c[3])
    print("MKTYPE(%s);" % map['ourname'], file=gen_c[4])
