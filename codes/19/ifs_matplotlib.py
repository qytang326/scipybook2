...
class IFSDesigner(HasTraits):
    ...
    add_button = Button("添加三角形")
    del_button = Button("删除三角形")
    save_button = Button("保存当前IFS")
    unsave_button = Button("删除当前IFS")
    clear = Bool(True)
    ifs_names = List()
    ifs_points = List()
    current_name = Str
    ...
    def save_data(self):               
        with file("ifs_matplotlib.data", "wb") as f:
            pickle.dump(list(zip(self.ifs_names, self.ifs_points.tolist())), f) 
