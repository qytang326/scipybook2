...
Positions = list(product(list(range(H)), list(range(W))))
...
BlockSizes = {key: (blocks[-1][0] + 1, blocks[-1][1] + 1) for key, blocks in list(Blocks.items())}
...
def get_moves(node, blocks):
    ...
    empty = {key for key, value in list(cells.items()) if value is None}
    possible_pos = set()
    for r, c in empty:
        for dr, dc in list(Direct.values()):
    ...
    for i in range(len(positions)):
        ...
        r, c = pos
        moves = block_moves[i]
        for move, offsets in list(moves.items()):
