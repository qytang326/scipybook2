...
def write_config(fn, config):
    with open(fn, "w") as f:
        for section, options_dict in list(config.items()):
            f.write("[{}]\n".format(section))
...
def add_vc9_reg(vc_dir):
    key = winreg.CreateKeyEx(HCU, r"Software\Microsoft\VisualStudio\9.0\Setup\VC")
    winreg.SetValueEx(key, "ProductDir", None, winreg.REG_SZ, vc_dir)
...
def remove_vc9_reg():
    try:
        winreg.DeleteKeyEx(HCU, r"Software\Microsoft\VisualStudio\9.0\Setup\VC")
        print("Removed")
    except WindowsError:
        print("key not exist")
